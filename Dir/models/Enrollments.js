        ...
        return new Promise(async(resolve, reject) => {
            try {
                _Database2.default.connect( err => {
                    ...
                    _Database2.default.query('SELECT COUNT(id) AS "boolean" FROM enrollments WHERE id_discord = (?) AND id_championship = (?)', [id_discord, id_championship], async(err, enrollment) => {
                        if (err) {
                            console.log(err);
                            return reject(err);
                        };
